...
public class EndToEndTests : IDisposable
{
    ...
    {
        _output.WriteLine("REVERT CHANGES");
        ...
        var responses = new Dictionary<Task<IResponse>, (string, RecordType)>();
        ...
        foreach (var (host, recordType) in requestsCombination)
        {
            ...
            {
                responses.Add(dnsClient.Resolve(host, recordType), (host, recordType));
            }
        }

        try
        {
            await Task.WhenAll(responses.Keys);
        }
        catch
        {
            foreach (var (response, request) in responses)
            {
                if (response.IsCanceled)
                {
                    _output.WriteLine($"Error in request {request.Item1} ({request.Item2})\n{response.Exception}");
                }
            }
            // throw;
            return;
        ...
        {
            var response = await responseTask.Key;
